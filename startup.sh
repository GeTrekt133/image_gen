...
tmux new -s comfy -d 'source /workspace/env.sh; cd /workspace/ComfyUI && python main.py --listen 0.0.0.0 --port 8188 > /workspace/comfy.log 2>&1'

echo -n "== [startup] жду :8188 "
for i in $(seq 1 60); do
  if curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8188/ 2>/dev/null | grep -q 200; then
    echo "→ ✅ ComfyUI поднят (http://0.0.0.0:8188)"; exit 0
  fi
  echo -n "."; sleep 3
done
